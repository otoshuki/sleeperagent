...
class RobosuiteEnv(Env):
    def __init__(self, env_name="PickPlaceCan", camera_names=["agentview", "robot0_eye_in_hand"], camera_height=32, camera_width=32, frame_stack=5, channels_first=True):
        super().__init__()
        ...
        if hasattr(self, '_last_obs') and self._last_obs is not None:
            ...
            if img is not None:
                if img.shape[0] == 3 and len(img.shape) == 3:  # (C, H, W)
                    img = np.transpose(img, (1, 2, 0))
                return img
        shape = self.observation_space['rgb_third'].shape
        if len(shape) == 3 and shape[0] == 3:
            shape = (shape[1], shape[2], shape[0])
        return np.zeros(shape, dtype=np.uint8)

    # def render(self, mode="rgb_array"):
    #     # Return the latest available observation image
    #     if hasattr(self, '_last_obs') and self._last_obs is not None:
    #         # You can choose which image to return, or concatenate both
    #         img = self._last_obs.get("rgb_third", None)
    #         if img is not None:
    #             return img
    #         img = self._last_obs.get("rgb_wrist", None)
